...
var listenBlocks = function(config) {
    ...
    if(error) {
      ...
      if(web3.isConnected()) {
        web3.let.getBlock(latestBlock, true, function(error,blockData) {
          if(error) {
...
var syncChain = function(config, nextBlock){
  if(web3.isConnected()) {
    ...
    while(nextBlock >= config.startBlock && count > 0) {
      web3.let.getBlock(nextBlock, true, function(error,blockData) {
        if(error) {
...
var prepareSync = function(config, callback) {
  ...
  oldBlockFind.exec(function (err, docs) {
    if(err || !docs || docs.length < 1) {
      ...
      if(web3.isConnected()) {
        var currentBlock = web3.let.blockNumber;
...
var runPatcher = function(config, startBlock, endBlock) {
  ...
  if (missingBlocks > 0) {
    ...
    while(count < config.patchBlocks && patchBlock <= endBlock) {
      ...
      web3.let.getBlock(patchBlock, true, function(error, patchData) {
        if(error) {
