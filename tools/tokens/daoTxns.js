...
var daoContract = web3.let.contract(daoABI);
  ...
  event({}, {fromBlock: creationEnd, toBlock: "latest"}).get( function(err, log) {
      if (err) {
        ...
        for (var l in log) {
          ...
          try {
            var block = web3.let.getBlock(log[l].blockNumber);
...
var patchTimestamps = function(collection) {
  mongoose.connection.on("open", function(err,conn) { 
    ...
    collection.find({timestamp: null}).forEach(function(doc) {
      setTimeout(function() {
        try {
          var block = web3.let.getBlock(doc.blockNumber);
        } catch (e) {
