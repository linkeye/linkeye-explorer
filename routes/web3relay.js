...
var newBlocks = web3.let.filter("latest");
var newTxs = web3.let.filter("pending");
...
exports.data = function(req, res){
  ...
  if ("tx" in req.body) {
    ...
    web3.let.getTransaction(txHash, function(err, tx) {
      if(err || !tx) {
        console.error("TxWeb3 error :" + err)
        if (!tx) {
          web3.let.getBlock(txHash, function(err, block) {
            if(err || !block) {
        ...
        var block = web3.let.getBlock(tx.blockNumber, function(err, block) {
          if (!err && block)
    ...
    if (options.indexOf("balance") > -1) {
      try {
        addrData["balance"] = web3.let.getBalance(addr);
    ...
    if (options.indexOf("count") > -1) {
      try {
         addrData["count"] = web3.let.getTransactionCount(addr);
      } catch (err) {
        console.error("AddrWeb3 error :" + err);
        addrData = {"error": true};
      }
    ...
    if (options.indexOf("bytecode") > -1) {
      try {
         addrData["bytecode"] = web3.let.getCode(addr);
    ...
    web3.let.getBlock(blockNumOrHash, function(err, block) {
      if(err || !block) {
        console.error("BlockWeb3 error :" + err)
        res.write(JSON.stringify({"error": true}));
      } else {
    ...
    */
    ...
    web3.let.getUncle(blockNumOrHash, uncleIdx, function(err, uncle) {
      if(err || !uncle) {
        console.error("UncleWeb3 error :" + err)
        res.write(JSON.stringify({"error": true}));
      } else {
    ...
    if (req.body.action == 'hashrate') {
      web3.let.getBlock('latest', function(err, latest) {
        if(err || !latest) {
          ...
          web3.let.getBlock(checknum, function(err, block) {
            if(err || !block) {
              console.error("StatsWeb3 error :" + err);
